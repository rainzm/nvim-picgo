...
local function callbackfn(job_id, data, _)
	...
	if data[1]:match("%[PicGo SUCCESS%]:") then
		...
		if vim.bo.filetype == "norg" then
			image_link = string.format(".image %s", data[2])
		else
			if default_config.image_name then
				image_link = string.format("![%s](%s)", vim.fn.fnamemodify(data[2], ":t:r"), data[2])
			else
				image_link = string.format("![](%s)", data[2])
			end
		end
		vim.fn.setreg(vim.v.register, image_link)
	end
end
